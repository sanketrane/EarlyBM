functions{
  ...
  real eps_function(real time, real[] params){
   ...
   real[] ODE_sys(real time,  real[] y, real[] parms, real[] rdata, int[] idata) {
     ...
     dydt[1] = rho * eps_function(time, parms) * (2 * y[7] + y[2] + y[1]) - rho * (1 - eps_function(time, parms)) * y[1] - lambda * y[1];
     // L1 large pre B cells in WT
     dydt[2] = rho * eps_function(time, parms) * (2 * y[8] + 2 * y[3]) + rho * (1 - eps_function(time, parms)) * (2 *  y[1]) - lambda * y[2];
     // U large pre B cells in WT
     dydt[3] = - rho * eps_function(time, parms) * y[3] + rho * (1 - eps_function(time, parms)) * (2* y[7] + 2* y[8] + y[2] + y[3]) - lambda * y[3];

     // L2 large pre B cells in dko
     dydt[4] = rho_dko * eps_function(time, parms) * (2 * y[9] + y[5] + y[4]) - rho_dko * (1 - eps_function(time, parms)) * y[4] - lambda_dko* y[4];
     // L1 large pre B cells in dko
     dydt[5] = rho_dko * eps_function(time, parms) * (2 * y[10] + 2 * y[6]) + rho_dko * (1 - eps_function(time, parms)) * (2 * y[4]) - lambda_dko * y[5];
     // U large pre B cells in dko
     dydt[6] = - rho_dko * eps_function(time, parms) * y[6] + rho_dko * (1 - eps_function(time, parms)) * (2* y[9] + 2* y[10] + y[5] + y[6]) - lambda_dko * y[6];
     
     // L2 small pre B cells in WT
     dydt[7] = theta * x_pos - (rho + lambda) * y[7];
     // L1 small pre B cells in WT
     dydt[8] = theta * x_neg - (rho + lambda) * y[8];

     // L2 small pre B cells in dko
     dydt[9] = theta_dko * x_pos - (rho_dko + lambda_dko) * y[9];
     // L1 small pre B cells in dko
     dydt[10] = theta_dko * x_neg - (rho_dko + lambda_dko) * y[10];
