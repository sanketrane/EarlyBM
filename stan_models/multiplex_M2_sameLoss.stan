functions{
  ...
  real eps_function(real time, real[] params){
    ...
    real value = 1.0/(1+(time/r_eps)^0.5);
    return value;
   }
   ...
   real[] ODE_sys(real time,  real[] y, real[] parms, real[] rdata, int[] idata) {
     ...
     real theta = (rho - lambda) * (y[7] + y[8])/(x_pos_wt + x_neg_wt);
     real theta_dko = (rho_dko - lambda) * (y[9] + y[10])/(x_pos_dko + x_neg_dko);
     ...
     dydt[1] = rho * eps_function(time, parms) * (2* y[7] + 2 * y[2] + y[1]) - rho * (1 - eps_function(time, parms)) * y[1] - (mu + lambda) * y[1];
     // L1 large pre B cells in WT
     dydt[2] = rho * eps_function(time, parms) * (2* y[8] + 2 * y[3] - y[2]) + rho * (1 - eps_function(time, parms)) * (2 *  y[1]) - (mu + lambda) * y[2];
     ...
     dydt[4] = rho_dko * eps_function(time, parms) * (2* y[9] + 2 * y[5] + y[4]) - rho_dko * (1 - eps_function(time, parms)) * y[4] - (mu_dko + lambda_dko) * y[4];
     // L1 large pre B cells in dko
     dydt[5] = rho_dko * eps_function(time, parms) * (2* y[10] + 2 * y[6] - y[5]) + rho_dko * (1 - eps_function(time, parms)) * (2 * y[4]) - (mu_dko + lambda_dko) * y[5];
     ...
     dydt[7] = theta * x_pos_wt + mu * (y[1] + y[2]) - (rho + lambda) * y[7];
     // L1 small pre B cells in WT
     dydt[8] = theta * x_neg_wt + mu * (y[3]) - (rho + lambda) * y[8];

     // L2 small pre B cells in dko
     dydt[9] = theta_dko * x_pos_dko + mu_dko * (y[4] + y[5]) - (rho_dko + lambda_dko) * y[9];
     // L1 small pre B cells in dko
     dydt[10] = theta_dko * x_neg_dko + mu_dko * (y[6]) - (rho_dko + lambda_dko) * y[10];
...
model{
  ...
  lambda_Log ~ normal(-3, 0.5);
  mu_Log ~ normal(-4, 0.5);
  mu_dko_Log ~ normal(-2, 0.5);
